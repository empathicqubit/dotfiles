...
for PORT in VGA-1 \
    DVI-I-1-1 DVI-I-2-2 DVI-I-3-3 DVI-I-4-4 \
    DVI-1 \
    HDMI-1 HDMI-2 HDMI-3 \
    DP-1 DP-2 DP-3 DP-4 DP-5 DP-6 ; do
    if [[ "$PORT" == "$INTERNAL_OUTPUT" ]] ; then
        continue
    fi
    if [ $(xrandr | grep "$PORT" | grep -c ' connected ') -eq 1 ]; then
            EXTERNAL_OUTPUT="$PORT"
    fi
done
